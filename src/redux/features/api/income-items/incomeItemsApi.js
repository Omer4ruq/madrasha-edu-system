...
const PDFDocument = ({ incomeItems, incomeHeads, fundTypes, academicYears, startDate, endDate }) => (
  <Document>
    <Page size="A4" orientation="landscape" style={styles.page}>
      ...
      <View style={styles.table}>
        <View style={styles.tableRow}>
          <Text style={[styles.tableHeader, { flex: 1 }]}>আয়ের ধরণ</Text>
        ...
        {incomeItems.map((item, index) => (
          <View key={item.id} style={[styles.tableRow, index % 2 === 1 && styles.tableRowAlternate]}>
            <Text style={[styles.tableCell, styles.tableCellCenter, { flex: 1 }]}>
              {incomeHeads.find((head) => head.id === item.incometype_id)?.incometype || 'অজানা'}
            </Text>
...
const IncomeItems = () => {
  ...
  const filteredIncomeItems = Array.isArray(filteredIncomeData?.results) ? filteredIncomeData.results : [];
      ...
      <div className="bg-black/10 backdrop-blur-sm rounded-2xl shadow-xl animate-fadeIn p-6">
        <div className="flex items-center justify-between p-4 border-b border-white/20">
          ...
          <div className="flex items-center space-x-4">
            ...
            >
              <option value="">সকল আয়ের ধরণ</option>
              {incomeHeads.map((head) => (
