...
const AddBehaviorMarks = () => {
  ...
  // API Queries
  const { data: institute, isLoading: instituteLoading } =
  ...
  const { data: allStudents, isLoading: studentsLoading } =
    useGetStudentActiveByClassQuery(selectedClass, { skip: !selectedClass });
  const { data: behaviorTypes, isLoading: behaviorTypesLoading } =
    useGetBehaviorTypeApiQuery();
  const { data: existingReports, isLoading: reportsLoading } =
    useGetBehaviorReportApiQuery();
  const [createBehaviorReport, { isLoading: isCreating }] =
    useCreateBehaviorReportApiMutation();
  ...
  const students = React.useMemo(() => {
    ...
    return allStudents.filter(
      (student) => student.admission_year_id === parseInt(selectedAcademicYear)
    );
  ...
  const hasAddPermission =
    groupPermissions?.some(
      (perm) => perm.codename === "add_student_behavior_report"
    ) || false;
  const hasChangePermission =
    groupPermissions?.some(
      (perm) => perm.codename === "change_student_behavior_report"
    ) || false;
  const hasViewPermission =
    groupPermissions?.some(
      (perm) => perm.codename === "view_student_behavior_report"
    ) || false;
  const hasDeletePermission =
    groupPermissions?.some(
      (perm) => perm.codename === "delete_student_behavior_report"
    ) || false;
      ...
      existingReports.forEach((report) => {
        ...
        if (examMatches && academicYearMatches) {
          report.behavior_marks.forEach((mark) => {
            if (
              students.some((student) => student.id === mark.student_id) &&
              ...
              if (
                ...
                updatedReports[mark.student_id][mark.behavior_type] !==
                  mark.mark.toString()
              ) {
                updatedReports[mark.student_id][mark.behavior_type] =
                  mark.mark.toString();
  ...
  const handleMarkChange = (studentId, behaviorTypeId, value) => {
    ...
    const behaviorType = activeBehaviorTypes.find(
      (bt) => bt.id === behaviorTypeId
    );
    ...
    if (value === "" || /^\d*\.?\d*$/.test(value)) {
      ...
      const isInvalid =
        ...
        value !== "." &&
        !isNaN(mark) &&
        (mark < 0 || mark > behaviorType.obtain_mark);
      const invalidKey = `${studentId}-${behaviorTypeId}`;
      const errorMessage = isInvalid
        ? `নম্বর ০ থেকে ${behaviorType.obtain_mark} এর মধ্যে হতে হবে`
        : "";
  ...
  const handleMarkSubmit = async (
    ...
    if (
      value === existingMark &&
      markStatus[studentId]?.[behaviorTypeId] === "success"
    ) {
      if (nextInputId) {
    ...
    if (value !== "" && value !== null && value !== undefined) {
      ...
      if (isNaN(mark) || mark < 0 || mark > behaviorType.obtain_mark) {
        ...
        showToast(
          `অবৈধ নম্বর! ০ থেকে ${behaviorType.obtain_mark} এর মধ্যে হতে হবে।`,
          "error"
        );
        setTimeout(
          () =>
            setMarkStatus((prev) => ({
              ...prev,
              [studentId]: { ...prev[studentId], [behaviorTypeId]: null },
            })),
          2000
        );
        return;
    ...
    activeBehaviorTypes.forEach((bt) => {
      ...
      if (
        currentMarkValue !== "" &&
        currentMarkValue !== null &&
        currentMarkValue !== undefined
      ) {
        const parsedValue = parseFloat(currentMarkValue);
        if (
          !isNaN(parsedValue) &&
          parsedValue >= 0 &&
          parsedValue <= bt.obtain_mark
        ) {
  ...
  const generateBehaviorReport = () => {
    ...
    if (!selectedClass || !selectedExam) {
      showToast(
        "প্রতিবেদন তৈরির জন্য শ্রেণী এবং পরীক্ষা নির্বাচন করুন।",
        "warning"
      );
      return;
    }
    ...
    if (instituteLoading) {
      showToast(
        "ইনস্টিটিউট তথ্য লোড হচ্ছে, অনুগ্রহ করে অপেক্ষা করুন!",
        "warning"
      );
      return;
    }
    ...
    const academicYearInfo = academicYears?.find(
      (year) => year.id === parseInt(selectedAcademicYear)
    );

    const classInfoText = classInfo
      ? `${classInfo.class_name} ${classInfo.shift_name} ${classInfo.section_name}`
      : "অজানা শ্রেণী";
    const examInfoText = examInfo ? examInfo.name : "অজানা পরীক্ষা";
    const academicYearInfoText = academicYearInfo
      ? academicYearInfo.name
      : null;
    ...
    const htmlContent = `
      ...
      ${
        ...
        <div class="class-details">
          ...
          ${
            academicYearInfoText
              ? `<p><strong>শিক্ষাবর্ষ:</strong> ${academicYearInfoText}</p>`
              : ""
          }
          <p><strong>প্রস্তুতির তারিখ:</strong> ${new Date().toLocaleDateString(
            "bn"
          )}</p>
        </div>
          ...
          <table>
            <thead>
              <tr>
                ...
                <th>রোল</th>
                ${activeBehaviorTypes
                  .map((bt) => `<th>${bt.name} (${bt.obtain_mark})</th>`)
                  .join("")}
              </tr>
            </thead>
            <tbody>
                ...
                <tr>
                  ...
                  ${activeBehaviorTypes
                    .map((bt) => {
                      ...
                      const isValidMark =
                        mark !== "" &&
                        !isNaN(parseFloat(mark)) &&
                        parseFloat(mark) <= bt.obtain_mark &&
                        parseFloat(mark) >= 0;
                      return `<td class="${
                        isValidMark ? "check-mark" : "cross-mark"
                      }">${mark || "-"}</td>`;
                    })
                    .join("")}
                </tr>
        ...
        <div class="summary">
          ...
          <p><strong>নম্বর প্রদানের হার (%):</strong> ${averageMarksPercentage.toFixed(
            2
          )}%</p>
        </div>
  ...
  const classOptions =
    classes?.map((cls) => {
      ...
      return {
        value: cls.id,
        label: `${cls.class_name}${shift}${section}`,
      };
    }) || [];
  ...
  return (
    <div className="min-h-screen mt-5">
      <div className="mx-auto">
        ...
        {toastMessage && (
          <div
            className={`fixed top-6 right-6 p-4 rounded-xl shadow-2xl z-50 toast-enter ${
              toastType === "success"
        ...
        <div className="glass-effect rounded-3xl p-8 mb-8 animate-fadeIn shadow-xl">
          <div className="flex items-center space-x-4 mb-8">
            ...
            <div>
              ...
              <p className="text-[#441a05] mt-1">
                ছাত্রদের আচরণ মূল্যায়ন করুন
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            ...
            <div className="animate-slideIn" style={{ animationDelay: "0.1s" }}>
              <label className="block text-sm font-semibold text-[#441a05] mb-1">
                একাডেমিক বছর
              </label>
              <div className="">
                <Select
            ...
            <div className="animate-slideIn" style={{ animationDelay: "0.2s" }}>
              ...
              <div className="">
                <Select
                  ...
                  onChange={(option) =>
                    setSelectedClass(option ? option.value : "")
                  }
            ...
            <div className="animate-slideIn" style={{ animationDelay: "0.4s" }}>
              <div className="p-4 rounded-xl">
                <div className="flex items-center justify-between mb-5">
                  <h3 className="text-sm font-semibold text-[#441a05]">
                    পরিসংখ্যান
                  </h3>
                  {selectedClass &&
                    selectedExam &&
                    ...
                    activeBehaviorTypes.length > 0 && (
                      <button
                        ...
                        className="flex items-center gap-2 px-3 py-2 bg-[#441a05] text-white rounded-lg transition-all duration-300 transform hover:scale-105 shadow-lg text-sm font-medium"
                ...
                <div className="flex justify-between gap-5 flex-nowrap">
                  ...
                  {activeBehaviorTypes.length > 0 && (
                    <p className="text-[#441a05] font-bold text-sm">
                      আচরণ ধরন: {activeBehaviorTypes.length}
                    </p>
                  )}
                  {selectedAcademicYear && (
                    <p className="text-[#441a05] font-bold text-sm">
        ...
        {selectedClass &&
          ...
          activeBehaviorTypes.length > 0 && (
            <div className="glass-effect rounded-3xl shadow-2xl animate-scaleIn overflow-hidden">
              <div className="p-6">
                <h3 className="text-xl font-bold text-[#441a05] mb-2">
                  আচরণ নম্বর প্রবেশ করুন
                </h3>
                <p className="text-[#441a05]">
                  Enter বা Tab দাবলে পরের ফিল্ডে যাবে এবং নম্বর সংরক্ষিত হবে।
                  খালি রেখে গেলে নম্বর মুছে যাবে।
                </p>
              </div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead className="bg-[#441a05]/10">
                    <tr>
                      <th className="p-4 text-left font-bold text-[#441a05] border-b border-gray-200">
                        ছাত্রের নাম
                      </th>
                      <th className="p-4 text-left font-bold text-[#441a05] border-b border-gray-200">
                        রোল নং
                      ...
                      {activeBehaviorTypes.map((bt) => (
                        <th
                          key={bt.id}
                          className="p-4 text-center font-bold text-[#441a05] border-b border-gray-200 min-w-[180px]"
                        >
                          <div>
                  ...
                  <tbody>
                    {students.map((student, studentIndex) => (
                      <tr
                        key={student.id}
                        className="table-row border-b border-gray-100 transition-all duration-300"
                      >
                        <td className="p-4 font-semibold text-[#441a05] text-sm">
                          {student.name}
                        </td>
                        ...
                        {activeBehaviorTypes.map((bt, behaviorIndex) => {
                          ...
                          const currentValue =
                            behaviorReports[student.id]?.[bt.id] || "";
                          const isInvalid =
                            invalidMarks[`${student.id}-${bt.id}`];
                          const currentMarkStatus =
                            markStatus[student.id]?.[bt.id];
                              ...
                              <div className="flex items-center justify-center">
                                <div className="relative input-container group">
                                  <input
                                    ...
                                    className={`w-24 p-3 pr-10 text-center border-2 outline-none rounded-xl bg-transparent input-field font-medium transition-all duration-300 ${
                                      isInvalid
                                        ? "border-red-400 bg-red-50 text-red-700 shadow-red-200"
                                        : currentMarkStatus === "success"
                                        ? "border-green-400 bg-green-50 text-green-700 shadow-green-200"
                                        : currentMarkStatus === "error"
                                        ? "border-red-400 bg-red-50 text-red-700 shadow-red-200"
                                        : currentMarkStatus === "loading"
                                        ? "border-blue-400 bg-blue-50 text-blue-700 shadow-blue-200"
                                  ...
                                  {currentValue &&
                                    ...
                                    (hasChangePermission ||
                                      ...
                                      <button
                                        onClick={() =>
                                          handleClearMark(student.id, bt.id)
                                        }
