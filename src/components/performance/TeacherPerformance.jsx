...
const TeacherPerformance = () => {
  ...
  const [createTeacherPerformance, { isLoading: isCreating }] =
    useCreateTeacherPerformanceApiMutation();
  const [patchTeacherPerformance, { isLoading: isUpdating }] =
    useUpdateTeacherPerformanceApiMutation();
  const {
    data: institute,
  ...
  const hasAddPermission =
    groupPermissions?.some(
      (perm) => perm.codename === "add_teacher_performance"
    ) || false;
  const hasChangePermission =
    groupPermissions?.some(
      (perm) => perm.codename === "change_teacher_performance"
    ) || false;
  const hasDeletePermission =
    groupPermissions?.some(
      (perm) => perm.codename === "delete_teacher_performance"
    ) || false;
  const hasViewPermission =
    groupPermissions?.some(
      (perm) => perm.codename === "view_teacher_performance"
    ) || false;
  ...
  const performanceData = useMemo(() => {
    ...
    performanceMetrics.forEach((metric) => {
      const perf = teacherPerformances.find(
        (p) => p.performance_name_id === metric.id
      );
...
const generatePDFReport = () => {
  ...
  const handleCheckboxChange = async (metricName) => {
    const actionPermission = performanceData[metricName]
      ? hasChangePermission
      : hasAddPermission;
    if (!actionPermission) {
    ...
    if (
      ...
      toast.error(
        "শিক্ষক, মাস এবং শিক্ষাবর্ষ নির্বাচন করুন এবং মেট্রিক্স লোড হয়েছে তা নিশ্চিত করুন।"
      );
      return;
    }
    ...
    try {
      const existingPerf = teacherPerformances.find(
        (p) => p.performance_name_id === metricId
      );
      const payload = {
      ...
      toast.error(
        `ত্রুটি: ${err.status || "অজানা"} - ${JSON.stringify(
  ...
  if (!hasViewPermission) {
    return (
      <div className="p-4 text-center text-red-500">
        এই পৃষ্ঠাটি দেখার অনুমতি আপনার নেই।
      </div>
    );
  }
  ...
  const renderPerformanceTable = () => {
    ...
    if (isMetricsLoading || isPerformanceLoading)
      return (
        <p className="p-4 text-[#441a05]/70 animate-fadeIn flex justify-center items-center h-full">
    ...
    if (metricsError)
      return (
        ...
        >
          মেট্রিক্স ত্রুটি: {metricsError.status || "অজানা"} -{" "}
          {JSON.stringify(metricsError.data || {})}
        </div>
      );
    if (performanceError)
      return (
        <div
          ...
          style={{ animationDelay: "0.4s" }}
        >
          কর্মক্ষমতা ত্রুটি: {performanceError.status || "অজানা"} -{" "}
    ...
    return (
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-white/20 !border-red-400">
          <thead className="">
            <tr>
              <th className="px-6 py-3 text-left text-xs font-medium text-[#441a05]">
                চেকবক্স
              </th>
              <th className="px-6 py-3 text-left text-xs font-medium text-[#441a05]">
                কর্মক্ষমতা মেট্রিক
        ...
        <div className="bg-black/10 backdrop-blur-sm border border-white/20 p-8 rounded-2xl mb-8 animate-fadeIn shadow-xl">
            ...
            <label className="flex items-center space-x-4 animate-fadeIn">
              ...
              <div className="w-full">
                <Select
                  ...
                  isLoading={isAcademicYearsLoading}
                  isDisabled={
                    isAcademicYearsLoading || isCreating || isUpdating
                  }
                  styles={selectStyles}
                  className="animate-scaleIn"
                  menuPortalTarget={document.body}
          ...
          {teachersError && (
            <div
              ...
              style={{ animationDelay: "0.4s" }}
            >
              শিক্ষক ত্রুটি: {teachersError.status || "অজানা"} -{" "}
          ...
          {academicYearsError && (
            <div
              ...
              style={{ animationDelay: "0.4s" }}
            >
              শিক্ষাবর্ষ ত্রুটি: {academicYearsError.status || "অজানা"} -{" "}
              {JSON.stringify(academicYearsError.data || {})}
            </div>
          )}
        </div>
      )}
